...
/**
 * A NumberValueMessage is a ValueMessage that can only take in numbers. The
 * number is also optionally accompanied by a unit in the metadata. <BR>
 * <BR>
 * The units if being told in relation to another should be x*y^-p with x being
 * the unit of measure, y being the unit being related to, and p being the
 * degree of y. If it is "first unit per second unit to x power" then the degree
 * has to be negative x. <BR>
 * <BR>
 * E.g: m*s^-2 <BR>
...
public class NumberValueMessage extends ValueMessage {
	...
	/**
	 * Constant unit for microseconds.
	 */
	public static final String MICROSECONDS_UNIT = "us";
	...
	/**
	 * Constructor for a number value message without a unit. Adds the type
	 * "number" to the "type" JsonArray.
	 * 
	 * @param source
	 ...
	 * 
	 * @param unit1
	 *            The first unit in the unit relation.
	 * @param unit2
	 *            The second unit in the unit relation that is multiplied with
	 *            the first.
	 * @param power
